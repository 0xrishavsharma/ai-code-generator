...
interface ExpandingInputProps {
  onSubmit: (prompt: string, file: File | undefined) => void
  onStop?: () => void
  isStreaming?: boolean
}

const ExpandingInput = forwardRef<ExpandingInputRef, ExpandingInputProps>(
  ({ onSubmit, onStop, isStreaming }, ref) => {
    ...
    const submit = (prompt: string, file: File | undefined) => {
      onSubmit?.(prompt, selectedFile)
      // Reset inputData after successful submission
      setInputData({ promptText: "", file: undefined })
    }

    const handleSubmit = (e: React.FormEvent) => {
      e.preventDefault()
      console.log("Prompt:", prompt)
      submit(prompt, selectedFile)
    }

    const buttonDisabled = prompt.length === 0 || isStreaming
